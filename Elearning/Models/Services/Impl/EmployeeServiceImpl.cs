    ...
    public class EmployeeServiceImpl : EmployeeServices
    {
        ...
        public int PolicyMark(int policyID, int version, int empID)
        {
                ...
                using (ElearningEntities e = new ElearningEntities())
                {
                    ...
                    List<Progress> progressOfEmployee = e.Progresses.Where(i => i.EmployeeID == empID).ToList();
                    List<Score> scoresOfGivenEmployee = null;
                    
                    foreach (Progress p in progressOfEmployee)
                    {
                        scoresOfGivenEmployee = e.Scores.Where(i => i.ProgressID == p.ProgressID).ToList();
                    }
